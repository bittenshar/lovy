...
const sendBulkPushNotification = async (userIds = [], options = {}) => {
  ...
  try {
    await sendBulkNotifications(userIds, options.title || 'Notification', options.message || 'You have a new notification', {
      type: options.data?.type || 'notification',
      priority: options.data?.priority || 'medium',
      notificationId: options.notificationId || '',
      actionUrl: options.data?.actionUrl || '',
      metadata: JSON.stringify(options.data?.metadata || {}),
    });
  } catch (error) {
